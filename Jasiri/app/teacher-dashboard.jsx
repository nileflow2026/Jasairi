 ...
 *
 * Design philosophy:
 *  - Lavender accent — creative, educational, trustworthy.
 *  - Shows class-wide progress, not just one child.
 *  - Skill bars show average performance across all students.
 *  - "Switch to Child View" lets teacher hand device to a learner.
...
const DEFAULT_SKILLS = [
  { label: "Memory", pct: 0, color: Colors.lavender },
  { label: "Language", pct: 0, color: Colors.blue },
  { label: "Motor Skills", pct: 0, color: Colors.green },
  { label: "Creativity", pct: 0, color: Colors.yellow },
];
...
// ─────────────────────────────────────────────────────────────
// Skill bar — JS-thread width animation
// ─────────────────────────────────────────────────────────────
function SkillBar({ skill, enterDelay }) {
  ...
  return (
    <View
      ...
      accessibilityLabel={`${skill.label}: ${skill.pct} percent class average`}
    >
...
export default function TeacherDashboardScreen() {
  ...
  const activeStudents = backendProgress.activeStudents ?? 0;
  const sessionsThisWeek = backendProgress.sessionsThisWeek ?? 0;
  const starsAwarded = backendProgress.starsAwarded ?? 0;
      ...
      >
        ...
        >
          <View style={styles.headerText}>
            <Text style={styles.headerEyebrow}>Teacher Dashboard</Text>
        ...
        {/* ── Stats row ── */}
        <View style={styles.statsRow} accessibilityLabel="Classroom statistics">
          <StatCard
            emoji="👥"
            label="Students"
        ...
        {/* ── Today's lessons ── */}
        <Card enterDelay={340}>
          <CardHeading emoji="📚" title="Today's Lessons" />
          {isLoading ? (
            <ActivityIndicator color={Colors.lavender} style={styles.loader} />
          ) : recentActivities.length === 0 ? (
            <Text style={styles.emptyText}>
              No lessons recorded yet today. Start a session!
            </Text>
          ) : (
        ...
        <Card enterDelay={480}>
          <CardHeading emoji="📈" title="Class Skill Averages" />
          {skills.map((skill, i) => (
            <SkillBar
              key={skill.label}
              skill={skill}
          ...
          <View style={styles.encourageRow}>
            ...
            <View style={styles.encourageBody}>
              <Text style={styles.encourageTitle}>
                Great teaching, {teacherName}!
              </Text>
              <Text style={styles.encourageText}>
                Your students' skills are up {weeklyGain} this week. Your
                dedication makes a real difference every day.
              </Text>
            </View>
          </View>
        </Card>
